<?php
/*
Plugin Name: PLUGIN TEST FOBIAS ESPECÍFICAS
Description: PLUGIN TEST FOBIAS ESPECÍFICAS
Version: 1.0
Author: Psycholoopgy
*/
...
register_activation_hook(__FILE__, function () {
    global $wpdb;
    $tabla_resultados = $wpdb->wp_fp6zg . 'fobias_resultados';
    $tabla_respuestas = $wpdb->wp_fp6zg . 'fobias_respuestas';
...
add_shortcode('fobias', function () {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['respuestas']) && isset($_POST['nombre']) && isset($_POST['apellido'])) {
        ...
        if ($total <= 20) {
            $diagnostico = "No hay fobias";
        } elseif ($total <= 35) {
            $diagnostico = "Fobias leve";
        } elseif ($total <= 60) {
            $diagnostico = "Fobias moderada";
        } else {
            $diagnostico = "Fobias alta";
        }
    ...
    <div class="psicotest-container">
        <h2>Test de Fobias Específicas</h2>
        <form method="POST">
